...
def findClusterMasterByFile(splunk_home:str) -> tuple:
	...
	'''
	splunk_home = wrc.normalizePathOS(str(splunk_home))
	apps_folder = wrc.normalizePathOS('etc/apps/')
	local_folder = wrc.normalizePathOS('etc/system/local/')
...
def findGUIDByFile(splunk_home:str) -> tuple:
	...
	'''
	splunk_home = wrc.normalizePathOS(str(splunk_home))
	etc_location = wrc.normalizePathOS(splunk_home + 'etc/'), # the comma here is NEEDED for a tuple of one value!
