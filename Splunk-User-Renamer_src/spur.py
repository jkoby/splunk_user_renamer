...
# Print Console Info
print("\n")
print(f"- SPUR({str(sys._getframe().f_lineno)}" +
      "): --- Splunk User Renamer: Starting ---- \n")
print(f"- SPUR({str(sys._getframe().f_lineno)}" +
      "): --- Splunk User Renamer: Timer Started ---- \n")
print(
    f"- SPUR({str(sys._getframe().f_lineno)}): Main Log Created at: ./logs/{main_log} -"
)
log_file.writeLinesToFile([
    f"SPUR({str(sys._getframe().f_lineno)}): --- Splunk User Renamer: Starting ----"
])
print("\n")
if arguments.args.test_run:
	print(
	    f"- SPUR({str(sys._getframe().f_lineno)}): ########################################### -"
	)
	print(
	    f"- SPUR({str(sys._getframe().f_lineno)}): TEST RUN - No actual changes will be made!! -"
	)
	print(
	    f"- SPUR({str(sys._getframe().f_lineno)}): ########################################### -"
	)
...
def generateMasterFolderSearchList():
	# FOLDERS
	print(
	    f"- SPUR({str(sys._getframe().f_lineno)}): Generating master search FOLDER list. These folders get backed up and renamed before any file contents are edited. -"
	)
	log_file.writeLinesToFile([
	    f"SPUR({str(sys._getframe().f_lineno)}): Generating master search FOLDER list. These folders get backed up and renamed before any file contents are edited."
	])
	...
	if not user_folders_list:
		print(
		    f"- SPUR({str(sys._getframe().f_lineno)}): No user folders found to rename. -"
		)
		log_file.writeLinesToFile([
		    f"SPUR({str(sys._getframe().f_lineno)}): No user folders found to rename."
		])
...
def generateMasterFileSearchList():
	# FILES
	print(
	    f"- SPUR({str(sys._getframe().f_lineno)}): Generating master search FILE list. This is done AFTER user folders have been backed up and renamed. -"
	)
	print(
	    f"- SPUR({str(sys._getframe().f_lineno)}): This means usernames in the PATH to these files will be new, but the content is still previous usernames. -"
	)
	log_file.writeLinesToFile([
	    f"SPUR({str(sys._getframe().f_lineno)}): Generating master search FILE list. This is done AFTER USER FOLDERS have been backed up and renamed, which...."
	])
	log_file.writeLinesToFile([
	    f"SPUR({str(sys._getframe().f_lineno)}):  ...usernames in the PATH to files in USER FOLDERS will be new, but the CONTENT still has previous usernames, which is ok."
	])
	...
	for fn in master_file_list:
		...
		if found[0]:
			master_file_path_list.extend(iter(found[1]))
	if master_file_path_list:
		return(master_file_path_list)
	print(
	    f"- SPUR({str(sys._getframe().f_lineno)}): No file names found to search in for users. -"
	)
	log_file.writeLinesToFile([
	    f"SPUR({str(sys._getframe().f_lineno)}): No file names found to search in for users."
	])
...
def determineCSV():
	...
	if not csv_path:
		...
		if not prefix_or_suffix_specified:
			print(
			    f"- SPUR({str(sys._getframe().f_lineno)}): No CSV folder specified and no prefix or suffix specified, therefore nothing to do. Exiting. -"
			)
			log_file.writeLinesToFile([
			    f"SPUR({str(sys._getframe().f_lineno)}): No CSV folder specified and no prefix or suffix specified, therefore nothing to do. Exiting."
			])
			...
			print(
			    f"- SPUR({str(sys._getframe().f_lineno)}): No CSV specified, creating rename dict from prefix and suffix. -"
			)
			log_file.writeLinesToFile([
			    f"SPUR({str(sys._getframe().f_lineno)}): No CSV specified, creating rename dict from prefix and suffix."
			])
		...
		print(
		    f"- SPUR({str(sys._getframe().f_lineno)}): CSV Folder Specified, attempting to read. -"
		)
		log_file.writeLinesToFile([
		    f"SPUR({str(sys._getframe().f_lineno)}): CSV Folder Specified, attempting to read."
		])
		try:
			...
			print(f"- SPUR({str(sys._getframe().f_lineno)}): Done. -")
			log_file.writeLinesToFile([f"SPUR({str(sys._getframe().f_lineno)}): Done."])
		except Exception as ex:
			print(
			    f"- WRC({str(sys._getframe().f_lineno)}): CSV Could not be read or processed. Exiting. -"
			)
			print(ex)
			log_file.writeLinesToFile([
			    f"({str(sys._getframe().f_lineno)}): CSV Could not be read or processed. Exiting."
			])
			log_file.writeLinesToFile([f"({str(sys._getframe().f_lineno)}): {str(ex)}"])
			spur_op_timer.stop()
			sys.exit()
...
def emailForUsernameCheck():
	...
	for u in user_folders_list:
		...
		for k, v in user_rename_dict.items():
			if str(u) != str(k):
				if orig_u_email_format:
					if not re.search(email_regex, str(k)): # provided original was not an email address
						new_k = f'{str(k)}@' + str(u.split('@')[1])
					if str(u) == str(k): # if they match now, add the modified uname to the list instead
						print(
						    f"- SPUR({str(sys._getframe().f_lineno)}): Original: {str(u)} but non-email username: {str(k)} specified for replacement. Using email version."
						)
						log_file.writeLinesToFile([
						    f"SPUR({str(sys._getframe().f_lineno)}): Original: {str(u)} but non-email username: {str(k)} specified for replacement. Using email version."
						])
					...
					if str(u) == str(k): # if they match now, add the modified uname to the list instead
						print(
						    f"- SPUR({str(sys._getframe().f_lineno)}): Original: {str(u)} but email username: {str(k)} specified for replacement. Using non-email version."
						)
						log_file.writeLinesToFile([
						    f"SPUR({str(sys._getframe().f_lineno)}): Original: {str(u)} but email username: {str(k)} specified for replacement. Using non-email version."
						])
...
def renameUsersInFiles():
	...
	log_file.writeLinesToFile([
	    f"SPUR({str(sys._getframe().f_lineno)}): The following files are being searched in for user renames."
	])
	file_failed_renames = []
	file_changes_dict = {} # store all file changes here: file_name : {k orig : v new}
	for f in master_file_path_list:
		print(f"- SPUR({str(sys._getframe().f_lineno)}):  - " + f + " -")
		log_file.writeLinesToFile(
		    [f"SPUR({str(sys._getframe().f_lineno)}" + "): 	- " + f])
	for f in master_file_path_list:
		tmp_changes_dict = {}
	...
	if not arguments.args.test_run:
		...
		log_file.writeLinesToFile([
		    f"SPUR({str(sys._getframe().f_lineno)}): All specified file modifications complete, successfuls will have a backup at: "
		    + arguments.args.backup_folder
		])
	else:
		...
		log_file.writeLinesToFile([
		    f"SPUR({str(sys._getframe().f_lineno)}): TEST RUN COMPLETE - NO CHANGES WERE MADE - Scroll up to see details on what would have changed"
		])
...
def renameUserFolders():
	...
	'''
	print(f"- SPUR({str(sys._getframe().f_lineno)}" +
	      "): Starting rename of user folders. -\n")
	log_file.writeLinesToFile([
	    f"SPUR({str(sys._getframe().f_lineno)}" +
	    "): Starting rename of user folders.\n"
	])
			...
			if str(u) == str(k):
				...
				if arguments.args.test_run:
					if os.path.exists(splunk_user_folders_path + str(v)):
						print(
						    f"- SPUR({str(sys._getframe().f_lineno)}): TEST MODE - New User Folder Already exists! Script would back it up and then delete it: "
						    + splunk_user_folders_path + str(v))
						log_file.writeLinesToFile([
						    f"SPUR({str(sys._getframe().f_lineno)}): TEST MODE - New User Folder Already exists! Script would back it up and then delete it: "
						    + splunk_user_folders_path + str(v)
						])
				elif os.path.exists(splunk_user_folders_path + str(v)):
					try:
						print(
						    f"- SPUR({str(sys._getframe().f_lineno)}): New User Folder Already exists! Backing it up and then deleting it first: "
						    + splunk_user_folders_path + str(v))
						log_file.writeLinesToFile([
						    f"SPUR({str(sys._getframe().f_lineno)}): New User Folder Already exists! Backing it up and then deleting it first: "
						    + splunk_user_folders_path + str(v)
						...
						print(
						    f"- SPUR({str(sys._getframe().f_lineno)}): Exiting, as had issue backing up or deleting: "
						    + splunk_user_folders_path + str(v))
						print(f"- SPUR({str(sys._getframe().f_lineno)}): Issue: {str(ex)}")
						log_file.writeLinesToFile([
						    f"SPUR({str(sys._getframe().f_lineno)}): Exiting, as had issue backing up or deleting: "
						    + splunk_user_folders_path + str(v)
						])
						log_file.writeLinesToFile(
						    [f"SPUR({str(sys._getframe().f_lineno)}): Issue: {str(ex)}"])
						spur_op_timer.stop()
						sys.exit()
				if wrc.renameFolder(splunk_user_folders_path + str(k), splunk_user_folders_path + str(v), create_backup=True, backup_to=arguments.args.backup_folder, test_run=arguments.args.test_run):
					print(f"- SPUR({str(sys._getframe().f_lineno)}): Original: " +
					      splunk_user_folders_path + str(k))
					print(f"- SPUR({str(sys._getframe().f_lineno)}): Renamed To: " +
					      splunk_user_folders_path + str(v))
					log_file.writeLinesToFile([
					    f"SPUR({str(sys._getframe().f_lineno)}): Original: " +
					    splunk_user_folders_path + str(k)
					])
					log_file.writeLinesToFile([
					    f"SPUR({str(sys._getframe().f_lineno)}): Renamed To: " +
					    splunk_user_folders_path + str(v)
					])
	...
	log_file.writeLinesToFile([
	    f"SPUR({str(sys._getframe().f_lineno)}): Rename complete, successfuls will have a backup at: "
	    + arguments.args.backup_folder + "\n"
	])
